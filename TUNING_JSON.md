...
| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `profile` | string | 否 | 调律名称。缺失时使用 `TUN`。字段名目前只读取小写 `profile`。 |
| `type` / `Type` | string | 否 | 绘制类型。空字符串、缺失或 `"octave"` 都表示 octave 模式；`"full"` 表示 full 模式。 |
| `offset` / `Offset` | number/string | 否 | octave 模式下表示瀑布整体显示偏移，单位为 cents；full 模式下表示标准音。full 模式缺失或空字符串时默认为 C4。 |
...
`profile` 不限制字符数量。界面中的 `TUN` 方块会在原有数值文本区域内显示调律名；如果名称过长，会用 6 个等宽字符的显示范围滚动显示。解析时仍会过滤不可打印 ASCII 字符；过滤后为空则使用 `TUN`。
...
full 模式下，`offset` 是标准音。缺失或空字符串时，默认使用 C4，也就是 MIDI `60.0`。

full 模式支持三类写法。

#### 1. 频率 Hz

频率必须是大于 0 的有限数字。可以写纯数字，也可以加 `Hz` 单位：
...
音名支持 `C D E F G A B`，可加 `#` 或 `b`。八度号采用 MIDI 标准：`C-1` 是 MIDI `0`，`C4` 是 MIDI `60`。

偏移音分可以省略；如果写偏移，`c` 单位可写可不写，也可以写成 `(c)`。

如果偏移音分绝对值超过 `50` cents，系统会自动折算到最近的标准 MIDI 音高，并保留不超过 `50` cents 的余量。也就是说，这类写法最终总会等价于“某个标准音名 +/- 不超过 50 cents”。
...
这些写法会直接换算为 MIDI 浮点音高。例如 `C#4+29.1` 对应 MIDI `61.291`。

大偏移示例：

```text
C4+129.1  等价于  C#4+29.1
C4-130c   等价于  B3-30c
```

#### 3. 相对 C-1 的音分值

如果要用“从 C-1 / MIDI 0 开始向上多少音分”来写，必须保留 `c` 单位，且数值必须非负：

```json
"offset": "6000c"
```

`6000c` 表示从 MIDI `0` 向上 `6000` cents，即 MIDI `60.0`。

注意：如果没有 `c`，纯数字字符串会被视为频率。例如 `"6000"` 表示 `6000Hz`，不是 `6000` cents。
...
- full 模式把纯数字字符串当 cents 写：full 下 `"6000"` 是 `6000Hz`；如果要表达从 C-1 起算的 `6000` cents，必须写 `"6000c"`。
- full 模式写负数 cents 起算值：相对 C-1 的 cents 写法必须非负。
